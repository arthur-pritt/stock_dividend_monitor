...
def validate_top_300(get_top_300):
    ...
    if len(get_top_300) !=300:
        raise ValueError(f" Expectedly exact rows, got {len(get_top_300)}")
...
def get_nasdaq_list():
    """
    Facade function that orchestrates the entire Nasdaq list cleaning, 
    matching, filtering, and API validation process. Function outside  files need to care about
    """

    logger.info("Starting to clean the 300 stock list...")
    #Checking if the file exists first to minimize downloading file at every run

    if NASDAQ_LIST_FILEPATH.is_file():
        logger.info("File Found, loading the file from the disk.....")
        nasdaq_clean_list=pd.read_csv(NASDAQ_LIST_FILEPATH)
    else:
        # 1. Gather the raw materials
        raw_data = load_nasdaq_data()
        # 2. Run the cleaning pipeline step-by-step
    ...
    logger.info("Pipeline Executed successfuly. Top 300 list is ready and fresh")
